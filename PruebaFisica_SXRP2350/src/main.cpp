#include <Arduino.h>
// =====================================================================
// main.cpp — Firmware de prueba física (Seeed XIAO RP2350)
// Sistema de extracción de aire caliente — Jaguar de México
// Candidato: Ing. Jair Molina Arce
//
// Versión para hardware real, derivada del firmware validado en la
// Fase 1 (main.py / fsm.py / adc_ntc.py / dip_switch.py / hbridge.py /
// fan.py y su equivalente Wokwi):
//
...
//
//   Filtrado ADC (hardware real): cada muestra que entra al buffer es
//   el promedio de una ráfaga de 8 lecturas consecutivas del ADC
//   (atenúa ruido de conmutación del ventilador/puente H); el buffer
//   circular de 8 muestras se promedia descartando máximo y mínimo
//   (trimmed mean), igual que adc_ntc.py.
//
//   Actuador FIT0803 (compuertas) via TB6612FNG canal A (PWMA fijo a 3V3):
//     Abrir : AIN1=LOW,  AIN2=HIGH, MOTOR_EN(STBY)=HIGH  → AO1=L, AO2=H
//     Cerrar: AIN1=HIGH, AIN2=LOW,  MOTOR_EN(STBY)=HIGH  → AO1=H, AO2=L
...
//
// BETA = 3435 K según datasheet del TEWA TT05-10KC8-1S-T105-1500.
// (En Wokwi se usaba 3950, el Beta del NTC genérico del simulador; la
// diferencia produce ~4°C de error a 50°C si no se corrige.)
// ---------------------------------------------------------------------
constexpr float BETA       = 3435.0;      // TEWA TT05 (datasheet)
...
// Fuera de este rango la lectura se considera inválida: NTC desconectado
// (nodo del divisor a Vcc → ADC saturado alto) o en corto (nodo a GND →
// ADC saturado bajo). En hardware real el ADC nunca entrega exactamente
// 0/4095, por eso la saturación se detecta con margen (ADC_SAT_MARGIN).
...
constexpr int   ADC_SAT_MARGIN = 8;       // raw < 8 o > 4087 → sensor ausente/corto
...
// Overrides manuales por puerto Serial (solo para demostración en la
// sesión en vivo: permiten forzar condiciones térmicas sin calentar
// físicamente el NTC hasta el setpoint). -1 / -999 = desactivado.
int   override_setpoint = -1;
float override_temp_int = -999.0;
...
// Ráfaga de 8 lecturas consecutivas promediadas. La primera se descarta:
// al conmutar el mux del ADC entre canales (GPIO26/GPIO27) el sample &
// hold necesita asentarse y la primera conversión arrastra residuo del
// canal anterior.
int readAdcBurst(int pin) {
  constexpr int N_BURST = 8;
  analogRead(pin);  // Descartar: asentamiento del mux
  long sum = 0;
...
float readTemperature(int pin, int* buffer, int& count, int& index) {
  ...
  if (raw < ADC_SAT_MARGIN || raw > (int)ADC_MAX - ADC_SAT_MARGIN) return TEMP_INVALID;
...
void run_fsm_step() {
  ...
  // Overrides manuales (demo): sustituyen la lectura del NTC mientras
  // estén activos. TEMP:AUTO / EXT:AUTO regresan al sensor físico.
  if (override_temp_int != -999.0) {
    temp_int = override_temp_int;
  }
  if (override_temp_ext != -999.0) {
    temp_ext = override_temp_ext;
  }

  // Validación de rango físico del NTC
...
void logTelemetry() {
  ...
  snprintf(buf, sizeof(buf),
           "[%010lu] TLM EST=%s INT=%.1fC%s EXT=%.1fC%s SP=%dC VENT=%s COMPUERTA=%s "
           "AIN1=%d AIN2=%d EN=%d",
           millis(), STATE_NAMES[currentState],
           temp_int, (override_temp_int != -999.0) ? "(OVR)" : "",
           temp_ext, (override_temp_ext != -999.0) ? "(OVR)" : "",
           readSetpoint(),
...
void setup() {
  ...
  Serial.println(" Prueba fisica XIAO RP2350 - Ing. Jair Molina Arce");
  Serial.println(" Comandos: SET:<C> | TEMP:<C>|AUTO | EXT:<C>|AUTO");
  Serial.println("=====================================================");
  ...
  if (Serial.available() > 0) {
    ...
    if (cmd.startsWith("SET:")) {
      ...
      if (valStr.equalsIgnoreCase("AUTO") || valStr.equalsIgnoreCase("OFF")) {
        override_temp_int = -999.0;
        Serial.println("Comando recibido: Temp interna manual desactivada (usa NTC)");
      } else {
        override_temp_int = valStr.toFloat();
        Serial.print("Comando recibido: Temp interna manual fijada a ");
        Serial.println(override_temp_int);
      }
