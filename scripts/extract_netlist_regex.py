...
def get_absolute_pin_coords(sym_x, sym_y, sym_rot, pin_rel_x, pin_rel_y):
    ...
    cos_t = math.cos(rad)
    sin_t = math.sin(rad)
    # KiCad symbol definition Y-axis is pointing UP, whereas schematic Y-axis is pointing DOWN.
    # Therefore, we must negate the relative Y coordinate of the pin.
    pin_rel_y = -pin_rel_y
...
for sym in placed_symbols:
    lib_id = sym['lib_id']
    # lib_id in schematic is the name in lib_symbols (without suffix)
    # or the name in the library (e.g. Device:C).
    # In lib_symbols block we stored base_name without _unit_convert suffix.
    # Placed symbol lib_id does not have _unit_convert suffix.
    # So we can look up lib_id directly in lib_symbols.
    pins = None
    if lib_id in lib_symbols:
        pins = lib_symbols[lib_id]
    else:
        # try case-insensitive or ends with
    ...
    if pins:
        ...
        print(f"Warning: Pins for {lib_id} ({sym['ref']}) not found.")

# Add labels
for ltype, name, lx, ly in labels:
    add_connection(lx, ly, ('LABEL', name))

# Snap coordinates and build adjacency graph for wires
...
# All coordinates in graph + connectivity
...
# Let's sort nets: those with labels first, then by name
labeled_nets = []
unlabeled_nets = []
